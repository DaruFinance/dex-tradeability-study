...
This repository contains the analysis engines and paper source for an empirical
...
- `engines/` — the core analysis and data-collection scripts: phased pipeline
...
- `chainscope/` — the supporting Python package: the per-fill cost model
  (`costs.py`), data storage and caching, chain definitions, provider clients,
  and shared utilities.
- `gap-studies/` — fifteen self-contained extended-study engines, one per
  ...
  streams).
- `paper/` — the LaTeX source of the working paper (`dex_study_v2.tex`).
